function animateSim(R, out)
% Animate the simulation output object generated by propagationSimulator.slx
%
% Copyright 2020 The MathWorks, Inc.


allPos = out.yout{1}.Values.Data;
xData = squeeze(allPos(:,1,:));
yData = squeeze(allPos(:,2,:));
...
[N,nPoints] = size(xData);
for i = 1:N
    hr(i) = rectangle(ax,'Position',[0 0 R R],'Curvature',[1 1],'FaceColor',[0 1 0],'EdgeColor',[0 1 0]);  %#ok<AGROW>
end
...
mp.XAxis.Limits = [0,nPoints];
mp.YAxis.Limits = [0,50];
hold on
s1 = stem(mp,0,0, 'LineWidth', 2, 'color', [0.00,0.45,0.74] , 'Marker', 'none');
s2 = stem(mp,0,0, 'LineWidth', 2, 'color', [0.39,0.83,0.07], 'Marker', 'none');
s3 = stem(mp,0,0, 'LineWidth', 2, 'color',[0.85,0.33,0.10], 'Marker','none');
hold off

for kk = 1:nPoints
%     ht.String = ['Time = ' num2str(out.yout{2}.Values.Time(kk))];
    for i = 1:length(hr)
        hr(i).Position = [xData(i,kk) yData(i,kk) R R];
    end
    ...
    s2.XData = 1:kk;
    s3.XData = 1:kk;
    s1.YData = status1Stem(1:kk);
    s2.YData = status2Stem(1:kk);
    s3.YData = status3Stem(1:kk);
    drawnow
end
